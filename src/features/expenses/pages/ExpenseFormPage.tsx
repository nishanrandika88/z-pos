...
export function ExpenseFormPage() {
  ...
  return <div className="space-y-4">
    ...
    <Card><CardHeader><h2 className="font-semibold">1. Expense basics</h2><p className="text-sm text-muted-foreground">Category controls which details appear below.</p></CardHeader><CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      <Field label="Category" required error={fieldErrors.categoryId}><select className="h-10 w-full rounded-md border bg-white px-3 text-sm" value={draft.categoryId} onChange={(event) => changeCategory(event.target.value)}><option value="">Select category</option>{(categoriesQuery.data ?? []).map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></Field>
      <Field label="Date and time" required error={fieldErrors.expenseDate}><Input type="datetime-local" value={toInputDateTime(draft.expenseDate)} onChange={(event) => setDraft({ ...draft, expenseDate: new Date(event.target.value).toISOString() })} /></Field>
      <Field label="Payment method" required error={fieldErrors.paymentMethod}><select className="h-10 w-full rounded-md border bg-white px-3 text-sm" value={draft.paymentMethod} onChange={(event) => setDraft({ ...draft, paymentMethod: event.target.value as ExpenseDraft["paymentMethod"] })}><option value="CASH">Cash</option><option value="CARD">Card</option><option value="BANK_TRANSFER">Bank transfer</option><option value="LANKAQR">LankaQR</option><option value="OTHER">Other</option></select></Field>
    </CardContent></Card>
...
function SalaryFields({ salary, employeeOptions, errors, onChange }: { salary: SalaryExpenseDraft; employeeOptions: EmployeeOption[]; errors: FieldErrors; onChange: (salary: SalaryExpenseDraft) => void }) {
  ...
  return <Card><CardHeader><h2 className="font-semibold">2. Salary details</h2><p className="text-sm text-muted-foreground">Net salary is calculated and used as the expense amount automatically.</p></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
    ...
    <Field label="Period start" required error={errors["salary.periodStart"]}><Input type="date" value={salary.periodStart} onChange={(event) => onChange({ ...salary, periodStart: event.target.value })} /></Field>
    <Field label="Period end" required error={errors["salary.periodEnd"]}><Input type="date" value={salary.periodEnd} onChange={(event) => onChange({ ...salary, periodEnd: event.target.value })} /></Field>
    <Field label="Basic salary" required error={errors["salary.basicSalary"]}><Input inputMode="decimal" value={salary.basicSalary} onChange={(event) => onChange({ ...salary, basicSalary: event.target.value })} /></Field>
    <Field label="Allowances" required helper="Enter 0.00 when none." error={errors["salary.allowances"]}><Input inputMode="decimal" value={salary.allowances} onChange={(event) => onChange({ ...salary, allowances: event.target.value })} /></Field>
    <Field label="Deductions" required helper="Enter 0.00 when none." error={errors["salary.deductions"]}><Input inputMode="decimal" value={salary.deductions} onChange={(event) => onChange({ ...salary, deductions: event.target.value })} /></Field>
    <Field label="Advances" required helper="Enter 0.00 when none." error={errors["salary.advancePayments"]}><Input inputMode="decimal" value={salary.advancePayments} onChange={(event) => onChange({ ...salary, advancePayments: event.target.value })} /></Field>
    <Field label="Payment date" optional><Input type="date" value={salary.paymentDate ?? ""} onChange={(event) => onChange({ ...salary, paymentDate: event.target.value })} /></Field>
    <div className="rounded-md border bg-muted/30 p-3"><p className="text-xs text-muted-foreground">Calculated net salary</p><p className="text-xl font-semibold">{net === "—" ? net : currency.format(Number(net))}</p></div>
  </CardContent></Card>;
}
...
function InventoryLines({ draft, setDraft, updateLine, inventory, fieldErrors }: { draft: ExpenseDraft; setDraft: React.Dispatch<React.SetStateAction<ExpenseDraft>>; updateLine: (index: number, patch: Partial<ExpenseLineDraft>) => void; inventory: Array<{ id: string; name: string; baseUnit: UnitType; averageCost: string; active: boolean }>; fieldErrors: FieldErrors }) {
  return <Card><CardHeader><div className="flex items-center justify-between"><div><h2 className="font-semibold">3. Purchased items</h2><p className="text-sm text-muted-foreground">Select an inventory match to auto-fill its base unit. Previous cost is shown only as a reference.</p></div><Button variant="outline" size="sm" onClick={() => setDraft({ ...draft, items: [...draft.items, emptyLine()] })}><Plus className="h-4 w-4" />Line</Button></div></CardHeader><CardContent className="space-y-3">
    ...
    <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={draft.updateInventory} onChange={(event) => setDraft({ ...draft, updateInventory: event.target.checked })} />Update matched inventory after approval</label>
  </CardContent></Card>;
}
...
function Field({ label: title, required, optional, helper, error, className = "", children }: { label: string; required?: boolean; optional?: boolean; helper?: string; error?: string; className?: string; children: React.ReactNode }) {
  return <label className={`space-y-1 text-sm ${className}`}><span className="font-medium">{title}{required ? <span className="text-destructive"> *</span> : null}{optional ? <span className="font-normal text-muted-foreground"> (Optional)</span> : null}</span>{children}{helper ? <span className="block text-xs text-muted-foreground">{helper}</span> : null}{error ? <span className="block text-xs text-destructive">{error}</span> : null}</label>;
}
