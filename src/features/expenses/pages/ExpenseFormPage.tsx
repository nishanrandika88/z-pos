...
export function ExpenseFormPage() {
  ...
  return <div className="space-y-4">
    ...
    {selectedCategory ? <>
      ...
      <FundingFields step={formType === "INVENTORY_PURCHASE" ? 4 : 3} draft={draft} setDraft={setDraft} employeeOptions={employeeOptionsQuery.data ?? []} total={totalsResult.totals?.grandTotal} error={fieldErrors.fundings} onSync={syncFunding} />
      {!expenseId ? <ReceiptFields step={formType === "INVENTORY_PURCHASE" ? 5 : 4} files={files} setFiles={setFiles} reviewedFileName={receiptQuery.data?.fileName} /> : null}
...
function CategoryFields({ formType, draft, setDraft, suppliers, employeeOptions, fieldErrors }: {
  ...
  return <Card><CardHeader><h2 className="font-semibold">2. {categoryTitle(formType)}</h2>{formType === "INVENTORY_PURCHASE" ? <p className="text-sm text-muted-foreground">Record the supplier, invoice, and notes for this purchase.</p> : null}</CardHeader><CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
    ...
    <Field label={payeeLabel} required={["RENT", "EQUIPMENT_REPAIR"].includes(formType)} optional={!["RENT", "EQUIPMENT_REPAIR"].includes(formType)} info={selectedSupplier ? "The selected saved supplier is recorded as the payee." : "The person or business that received the payment. It is saved for expense history, search, reports, and duplicate checks."} helper={selectedSupplier ? "Taken from the saved supplier." : undefined} error={fieldErrors.payee}><Input disabled={Boolean(selectedSupplier)} value={selectedSupplier?.name ?? draft.payee ?? ""} onChange={(event) => setDraft({ ...draft, payee: event.target.value })} /></Field>
...
function FundingFields({ step, draft, setDraft, employeeOptions, total, error, onSync }: { step: number; draft: ExpenseDraft; setDraft: React.Dispatch<React.SetStateAction<ExpenseDraft>>; employeeOptions: EmployeeOption[]; total?: string; error?: string; onSync: () => void }) {
...
function categoryTitle(formType: ExpenseFormType) { return ({ GENERAL: "General expense details", SALARY: "Salary details", RENT: "Rent details", UTILITY: "Utility bill details", INVENTORY_PURCHASE: "Purchase details", EQUIPMENT_REPAIR: "Equipment or repair details" })[formType]; }
