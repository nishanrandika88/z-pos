...
function Field({ label: title, required, optional, info, helper, error, className = "", children }: { label: string; required?: boolean; optional?: boolean; info?: string; helper?: string; error?: string; className?: string; children: React.ReactNode }) {
  return <label className={`space-y-1 text-sm ${className}`}><span className="flex items-center gap-1 font-medium">{title}{required ? <span className="text-destructive"> *</span> : null}{optional ? <span className="font-normal text-muted-foreground"> (Optional)</span> : null}{info ? <InfoHint text={info} /> : null}</span>{children}{helper ? <span className="block text-xs text-muted-foreground">{helper}</span> : null}{error ? <span className="block text-xs text-destructive">{error}</span> : null}</label>;
}

function InfoHint({ text }: { text: string }) {
  return <button type="button" className="group relative inline-flex shrink-0 cursor-help rounded-full text-brand-forest outline-none focus-visible:ring-2 focus-visible:ring-brand-forest/40" aria-label={`Information: ${text}`} onClick={(event) => { event.preventDefault(); event.stopPropagation(); }}><Info className="h-3.5 w-3.5" aria-hidden="true" /><span className="pointer-events-none invisible absolute left-0 top-full z-50 mt-1 w-64 max-w-[calc(100vw-3rem)] rounded-md bg-slate-950 px-3 py-2 text-left text-xs font-normal leading-relaxed text-white opacity-0 shadow-lg transition-opacity group-hover:visible group-hover:opacity-100 group-focus:visible group-focus:opacity-100" aria-hidden="true">{text}</span></button>;
}
